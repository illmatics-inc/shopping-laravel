...
class User extends Authenticatable
{
    ...
    protected static function boot()
    {
        ...
        self::deleting(function (User $user) {
            optional($user->image_path)->deleteFile();
    ...
    public function setImagePathAttribute(?UploadedFile $value)
    {
        if (isset($this->image_path)) {
            $this->image_path->deleteFile();
        }
        if (is_null($value)) {
            $this->attributes['image_path'] = null;
            return;
        }
        $this->attributes['image_path'] = UserImage::fromUploadedFile($value);
    }

    /**
     * @param  string|null  $value
     * @return UserImage|void
     * @throws \Exception
     */
